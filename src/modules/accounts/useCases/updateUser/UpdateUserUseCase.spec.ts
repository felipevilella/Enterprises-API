...
describe('Update user', () => {
  ...
  it('should be able to update user by admin', async () => {
    ...
    const dataUser: IUpdateUserDTO = {
      ...
      city_residence: 'Contagem',
    };

    const result = await updateUserUseCase.execute(dataUser);

    expect(result).toHaveProperty('id');
  });

  it('should not be able to update user by outer user basic', async () => {
    expect(async () => {
      const outerUserBasic: ICreateUserDTO = {
      ...
      await updateUserUseCase.execute(dataUser);
